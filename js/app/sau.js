...
ss.extendObj( ss.SimpleUpload.prototype, {
    ...
    */
    _errorFinish: function( fileObj, status, statusText, response, errorType, progBox, sizeBox, pctBox, abortBtn, removeAbort ) {
        "use strict";
        //  alert( 'Server error response: ' + response  + ':' + errorType  + ':' + statusText);
...
//  + '_' +  JSSHOP.getUnixTimeStamp()
var doAIU = function(cmpPfix, tmpBcb, tmpMfldr, tmpItemID) {
try {
	var btn, tmpDAIurl = null;
      var btn = document.getElementById(cmpPfix + 'uploadBtn'),
	...
	var uploader = null;
  	uploader = new ss.SimpleUpload({
        ...
        url: tmpDAIurl,
        name: 'uploadfile',
        dropzone: cmpPfix + 'dragbox', // ID of element to be the drop zone
        multipart: true,
        hoverClass: 'hover',
        focusClass: 'focus',
        responseType: 'json',
        ...
        onComplete: function( filename, response ) {
            ...
            if ( response.success === true ) {
			if(response.in){window[tmpBcb](response.in);}
        ...
        onError: function(filename, type, status, statusText, response, uploadBtn, size) {
            ...
            msgBox.innerHTML = 'E Unable to upload file: ' + JSON.stringify(response);
