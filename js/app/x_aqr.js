...
function putalign(x, y)
{
    ...
    qrframe[x + width * y] = 2;
    for (j = -2; j < 2; j++) {
        qrframe[(x + j) + width * (y - 2)] = 3;
        qrframe[(x - 2) + width * (y + j + 1)] = 3;
        qrframe[(x + 2) + width * (y + j)] = 3;
        qrframe[(x + j + 1) + width * (y + 2)] = 3;
    }
    for (j = 0; j < 2; j++) {
        setmask(x - 1, y + j);
        setmask(x + 1, y - j);
...
// insert finders - black to frame, white to mask
    for (t = 0; t < 3; t++) {
        ...
        for (x = 2; x < 4; x++) {
            qrframe[(y + x) + width * (k + 2)] = 2;
            qrframe[(y + 2) + width * (k + x + 1)] = 2;
            qrframe[(y + 4) + width * (k + x)] = 2;
            qrframe[(y + x + 1) + width * (k + 4)] = 2;
        }
    }
...
function doNuqr(qStr, qSize) {

    tCW = getViewportWidth();
    tCH = getViewportHeight();
    var wd = tCH;
    if(tCH > tCW) {
    wd = tCW;
    }
    ...
    qrc.canvas.height = ht;
    qfw = genframe(qStr);
    qjsts = JSON.stringify(qfw);
    qqfa = qjsts.replace(/1,1,1,1,1,1,1/gi,"3,3,3,3,3,3,3");
    // qf = JSON.parse(qqfa);
    qf = qfw;
    ...
    for( i = 0; i < width; i++ ) {
		...
		case 1:
    		destCtx.fillStyle = '#000000';
              destCtx.fillRect(px*(4+i),px*(4+j),px,px);
    		qrc.fillStyle = '#990000'; // 000090
            qrc.fillRect(px*(4+i),px*(4+j),px,px);
		break;
		case 2:
    		destCtx.fillStyle = '#000000'; // 000090
              destCtx.fillRect(px*(4+i),px*(4+j),px,px);
    		qrc.fillStyle = '#009910';
            qrc.fillRect(px*(4+i),px*(4+j),px,px);
		break;
		case 3:
    		destCtx.fillStyle = '#000000';
              destCtx.fillRect(px*(4+i),px*(4+j),px,px);
    		qrc.fillStyle = '#000050';
            qrc.fillRect(px*(4+i),px*(4+j),px,px);
		break;
		case 4:
    		destCtx.fillStyle = '#000000'; // 000090
...
*/
 ...
 // clipCtx.shadowColor = "#000000";

// Specify the shadow offset.
 // clipCtx.shadowOffsetX = 2;
 // clipCtx.shadowOffsetY = 2;
...
pclipCtx.drawImage(destinationCanvas, 0, 0);
// pclipCtx.globalCompositeOperation = 'source-in';
// pclipCtx.drawImage(clipCanvas, 0, 0);
// clipCtx.restore();
