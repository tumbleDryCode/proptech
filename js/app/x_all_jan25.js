     ...
     // make scroll bars always visible
       content_style: "body {overflow-y: scroll !important;}",
        plugins: [
            ...
            'emoticons template paste textcolor colorpicker textpattern textshadow imagetools code bgcolorpicker mediapop'
        ],
        ...
        toolbar2: 'print preview media | forecolor backcolor emoticons imagetools fontsizeselect textpattern | fontselect | code | template | bgcolorpicker | mediapop | textshadow',
        image_advtab: true,
        templates: [
...
function openMediaPickerDialog() { 
    ...
    var imagesHtml = '<div id="mp_tab_image_content" style="display:none;min-height:350px;max-height:400px;overflow:auto;">';
        imagesHtml += '<div style="position: sticky; top: 0; background: white; z-index: 1; padding: 5px; border-bottom: 1px solid #ddd;"><input type="checkbox" id="mp_set_bg"> <label for="mp_set_bg">Set as Background</label> <input type="checkbox" id="mp_rounded"> <label for="mp_rounded">Rounded Corners</label></div>';
...
function openTextShadowDialog() {
    var dialogHtml = `
        <div style="padding:20px;">
            <h3>Text Shadow</h3>
            <label>Horizontal Offset (px): <input type="number" id="ts_h" value="2"></label><br>
            <label>Vertical Offset (px): <input type="number" id="ts_v" value="2"></label><br>
            <label>Blur Radius (px): <input type="number" id="ts_b" value="4"></label><br>
 
                        <label style="display:block;margin-bottom:5px;">Color: <input type="text" id="ts_c" value="#2196f3" data-coloris style="width:100px;"></label>
            <div id="ts_colorPreview" style="width:100%;height:40px;border-radius:8px;margin:10px 0;background:#2196f3;border:1px solid #ddd;"></div>

            <button id="ts_apply">Apply</button>
            <button id="ts_remove">Remove Text Shadow</button>
        </div>
    `;
    JSSHOP.ui.popAndFillLbox(dialogHtml);
    setTimeout(function() {
        document.getElementById('ts_apply').onclick = function() {
            var h = document.getElementById('ts_h').value;
            var v = document.getElementById('ts_v').value;
            var b = document.getElementById('ts_b').value;
            var c = document.getElementById('ts_c').value;
        ...
        document.getElementById('ts_remove').onclick = function() {
            var editor = tinymce.activeEditor;
            var selected = editor.selection.getContent();
            if (selected) {
        ...
        document.getElementById('ts_c').addEventListener('input', function() {
            document.getElementById('ts_colorPreview').style.backgroundColor = this.value;
        });
    }, 100);
}
