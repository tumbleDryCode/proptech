...
fnishCntLoad = function() {
 ...
 tmpDOqs = null;
tmpDOqs = {};
tmpDOqs["ws"] = "where uid=? and prtype=?";
tmpDOqs["wa"] = [quid, "5"];
tmpDOqs["l"] = 45;
...
// oiaSTR = "select p.*, pd.pd_prptitle, pd.pd_prpdesc from property p, propdescs pd where p.uid = " + quid+ " and p.prtype = '5' and pd.pd_prpid = p._id and pd.pd_prptlng = '" +  usrlang + "' order by p._id desc limit 20";
oiaSTR = "SELECT p.*, u.u_icon, u.u_fullname, COALESCE(pd_user.pd_prptitle, pd_def.pd_prptitle) AS pd_prptitle, COALESCE(pd_user.pd_prpdesc,  pd_def.pd_prpdesc)  AS pd_prpdesc FROM property p JOIN quser u ON p.uid = u._id LEFT JOIN propdescs pd_user ON pd_user.pd_prpid = p._id AND pd_user.pd_prptlng = '" +  usrlang + "' LEFT JOIN propdescs pd_def ON pd_def.pd_prpid = p._id AND pd_def.pd_prptlng = '" + deflang + "' WHERE p.uid = " + quid + " AND p.prtype = '5' ORDER BY RAND() LIMIT 20";
