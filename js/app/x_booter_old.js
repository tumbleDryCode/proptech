...
JSSHOP.ui.setTinnerHTML("dvHdrTtl", "");
// setCartIplugs();
if(cid == 0) {
} else {

    if(pid.indexOf("aa-edit") != -1) {
        tmpStrLgotxt = "Admin: " + JSSHOP.shared.getFieldVal("c_name", "recamby");
        } else {
        tmpStrLgotxt = JSSHOP.shared.getFieldVal("c_name", "recamby");
        }
        // JSSHOP.ui.setTinnerHTML("dvHdrTtl", tmpStrLgotxt);
...
if(boolShwLogoHdr == "y") {
tCTV = document.getElementById("c_title").value;
tLIV = document.getElementById("c_logoimg").value;
tDVstr = "<table style=\"margin:0 auto;align:center;max-width:90%\"><tr><td>";
if((tLIV == "noQvlaue") || (tLIV == "noQvalue")) {
tDVstr += "<div style=\"min-height:50px; min-width:50px;max-height:55px; max-width:55px;float:left;\" class=\"txtSizeFifty txtBold txtClrHdr bkgdClrNrml\">" + tCTV.substring(0, 1) + "</div>";
} else {
tDVstr += "<div style=\"background-image: url('images/pimgs/s_thumb" + JSSHOP.shared.getFieldVal("c_logoimg", "deflogo.png") + "'); background-repeat: no-repeat; background-size: contain;min-height:50px; min-width:50px;max-height:55px; max-width:55px;float:left;\" class=\"txtSizeFifty txtBold txtClrHdr bkgdClrNrml\"></div>";
}
tDVstr += "<div class=\"txtBold txtClrHdr\" style=\"padding-top:10px;\">" + tCTV + "</div>";
tDVstr += "</td></tr></table>";
document.getElementById('dvLogoImgHdr').innerHTML = tDVstr;
}
...
try {} catch(e) {    
    alert("mfnishCntLoad: " + e);
    // console.log("mfnishCntLoad: " + e);
    }
tppid = currUrlArr.pid;
switch(tppid) {
case "aa-edit-user":
case "aa-edit-message":
case "login":
case "register":
case "aa-edit-shop":
case "aa-edit-item":
...
case "aa-add-new-shop":
case "aa-contactus": 
case "aa-remove":   
case "aa-show-aapp":
case "aa-edit-svs-category":   
case "aa-show-svs-category":
tCanSPrtPrts = "n";
tCanSPrtSrvcs = "n"
break;
...
break;
case "aa-show-place":
    // JSSHOP.ui.showHideElement("dvMMPList", "show");
...
break; 
case "aa-show-psearch":
if((currUrlArr.q) || (JSSHOP.ads.getPrtsPrefCC("prti") !== "noQvalue")) {
    tCanSPrtPrts = "m";
} else {
    tCanSPrtPrts = "y";
}
...
// hide the parts and services menus if no parts or services
// if((JSSHOP.ads.getPrtsPrefCC("mk") == "noQvalue") && (JSSHOP.ads.getPrtsPrefCC("qloc") == "noQvalue")) {
//    tCanSPrtSrvcs = "n";
// }
if(currUrlArr.cid) {
    tCanSPrtSrvcs = "n";
}
if((tCanSPrtPrts == "n") && (tCanSPrtSrvcs == "n")) {
    console.log("mfnishCntLoad.dvMMainParts.tCanSPrtPrts: " + tCanSPrtPrts + " tCanSPrtSrvcs: " + tCanSPrtSrvcs);
JSSHOP.ui.showHideElement("dvMMainParts", "hide");
} else {
if((tCanSPrtPrts == "y") || (tCanSPrtPrts == "m")) {
    console.log("mfnishCntLoad.dvMMainParts.tCanSPrtPrts: " + tCanSPrtPrts + " tCanSPrtSrvcs: " + tCanSPrtSrvcs);
// doDefMnuLd();
if(document.getElementById("dvPartsList")) {
// check if parts object is null
if(dvPartsList.innerHTML == "") {
    console.log("mfnishCntLoad.dvPartsList.innerHTML: blank");
JSSHOP.shared.doPartsLinks();
} else {

    tCIAPid = currUrlArr.pid;
    if(tCIAPid.indexOf("-edit") != -1) {
        console.log("mfnishCntLoad.dvPartsList.-edit: " + tCIAPid);

        // dvPartsList.innerHTML == "";
        JSSHOP.shared.doPartsLinks();

    } else {
        if(document.getElementById("btnTglSBPtsL") !=  null) {
            console.log("mfnishCntLoad.btnTglSBPtsL.innerHTML: not blank: " + dvPartsList.innerHTML);
            // dvPartsList.innerHTML == "";
            JSSHOP.shared.doPartsLinks();

        } else {
            console.log("mfnishCntLoad.dvPartsList.innerHTML: not blank: " + dvPartsList.innerHTML);

        JSSHOP.shared.doPartsWords(50, "dvPartLinks");
    

    if(JSSHOP.ads.getPrtsPrefCC("prti") == "noQvalue") {
        if(dvTBPList.style.display == "block") {
    ...
    }  // end of if btnTglSBPtsL
    


    }  // end of tCIAPid.indexOf("-edit") != -1


    if(tCanSPrtPrts == "m") {
        // minimize dvMMPList setting usrprefs tglBPrtsL
    // if dvTBPList style display is visible then hide it
    if(dvTBPList.style.display == "block") {
          JSSHOP.ui.toggleNuModule('btnTglBPrtsL','dvTBPList');
    } 
    }
    // JSSHOP.shared.doPartsWords(50, "dvPartLinks"); 
    JSSHOP.ui.showHideElement("dvMMainParts", "show");

}  // dvPartsList not blank
} // end of if dvPartsList
// } else 
   //  JSSHOP.ui.showHideElement("dvMMPList", "hide");
} else { //  tCanSPrtPrts == "n"
console.log("mfnishCntLoad.dvMMPList.hide2: " + tCanSPrtPrts + " tCanSPrtSrvcs: " + tCanSPrtSrvcs);
JSSHOP.ui.showHideElement("dvMMPList", "hide");
// setTimeout("JSSHOP.ads.doGenericPlug('mpmenu',3,'dvPartLinks')", 1500);
}
if(tCanSPrtSrvcs == "y") {
JSSHOP.ui.showHideElement("dvPartsSrvcs", "show");
} else {
JSSHOP.ui.showHideElement("dvPartsSrvcs", "hide");
}

}
mnuT.innerHTML = "";
// JSSHOP.shared.setOGtags();
if((currUrlArr.cid) || (currUrlArr.tpid)) {
    if(currPartsObj["pcats"]) {
doNuMMenuLd("doMnuFnsh");
    } else {
        JSSHOP.ajax.doNuAjaxPipe("menu", "misc/x_cp_" + usrlang + ".txt", JSSHOP.shared.setCurPrtsObj);
    }
        
}
setTimeout("JSSHOP.ads.doGenericPlug(pid,3,'dvPartLinks')", 1500);
